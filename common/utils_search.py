...
def extract_agent_response_and_urls(project_client, thread_id, query):
    ...
    For use with synchronous AIProjectClient.
    """
    extracted_urls = []
    agent_response_text = ""
...
async def extract_agent_response_and_urls_async(project_client, thread_id, query):
    """
    Extracts the agent's text response and cited URLs from the last agent message in a thread.
    For use with asynchronous AIProjectClient (from azure.ai.projects.aio).
    """
    extracted_urls = []
    agent_response_text = ""

    last_agent_message = await project_client.agents.messages.get_last_message_by_role(
        thread_id=thread_id,
        role=MessageRole.AGENT
    )
